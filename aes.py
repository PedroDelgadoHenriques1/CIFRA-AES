...
def sub_bytes(state):
    ...
    for row in state:
        new_row = []
        for cell in row:
            row_idx = (cell & 0xF0) >> 4  # Primeiro nibble (linha da S-box)
            col_idx = cell & 0x0F         # Segundo nibble (coluna da S-box)
            new_row.append(s_box[row_idx][col_idx])
...
def substitute_bytes(state, s_box):
    """Substitui os bytes do estado usando a S-box."""
    for i in range(4):
        for j in range(4):
            byte = state[i][j]
            row = (byte >> 4)  # Obtém a linha (4 bits mais significativos)
            col = (byte & 0x0F)  # Obtém a coluna (4 bits menos significativos)
            
            # Verifica se row e col estão dentro do intervalo da S-box
            if row < 0 or row >= len(s_box) or col < 0 or col >= len(s_box[0]):
                raise ValueError(f"Índices fora do intervalo: row={row}, col={col}, byte={byte}")

            state[i][j] = s_box[row][col]  # Substitui pelo valor da S-box
    return state
...
# Substitui bytes
new_state = substitute_bytes(state, s_box)

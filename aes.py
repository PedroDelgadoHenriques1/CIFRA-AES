...
print("\nMensagem convertida em Transposta:")
print(mensagem_transposta)

print("\nChave convertida em Transposta:")
print(chave_transposta)
...
# Definindo a S-box (exemplo da S-box do AES)
Sbox = [
    ...
    0x9B, 0x1E, 0x87, 0xE6, 0xED, 0xD3, 0x34, 0x6E,
]

# Função para rotacionar a palavra
def rotate_word(word):
    return word[1:] + word[:1]

# Função para substituir os bytes usando a S-box
def substitute_bytes(word):
    return [hex(Sbox[int(byte, 16)]).upper()[2:].zfill(2) for byte in word]

# Função para fazer a operação XOR entre duas palavras
...
def expand_key(key_matrix):
    ...
    # Converte a chave em uma lista de palavras
    key_words = [key_matrix[:, i].tolist() for i in range(4)]
    expanded_key = key_words.copy()

    for i in range(4, 44):
        if i % 4 == 0:  # Se i for múltiplo de 4
            temp = substitute_bytes(rotate_word(expanded_key[i - 1]))
            temp[0] = hex(int(temp[0], 16) ^ Rcon[i // 4 - 1]).upper()[2:].zfill(2)
            new_word = xor_words(expanded_key[i - 4], temp)
        else:  # Para outros casos
            new_word = xor_words(expanded_key[i - 1], expanded_key[i - 4])

        expanded_key.append(new_word)

    return expanded_key

# Definindo a chave original como uma matriz 4x4 (a chave deve ser transposta se necessário)
key_matrix = np.array([
    [0xAB, 0x8B, 0x98, 0x35],
    [0x05, 0x40, 0x7F, 0xF1],
    [0x18, 0x3F, 0xF0, 0xFC],
